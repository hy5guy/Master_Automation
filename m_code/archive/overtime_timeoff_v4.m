...
in List.Contains(PeriodLabels, "11-25") and
    not List.Contains(PeriodLabels, "12-25") else
        ...
        List.Contains(Cols, "11-25") and
    not List.Contains(Cols, "12-25"),
    ...
    PriorAccruals = if HasTimeCategory then let Keep = Table.SelectRows(
        PriorCsv,
    ...
    Typ = Table.TransformColumnTypes(Ren, {{"Total", type number}}) in Typ else
#table({"Metric", "Period", "Total"}, {}),

          PriorAccrualsNorm = let Trimmed = Table.TransformColumns(
        PriorAccruals,
        {{"Metric", Text.Trim, type text}, {"Period", Text.Trim, type text}}),
    WithLabel = Table.AddColumn(Trimmed, "PeriodLabel",
                                each let mm = Text.Start([Period], 2),
                                yy = Text.End([Period], 2),
                                y = Number.FromText("20" & yy),
                                m = Number.FromText(mm),
                                d = #date(y, m, 1) in Date.ToText(d, "MM-yy"),
                                type text),
    KeepCols = Table.SelectColumns(WithLabel, {"Metric", "PeriodLabel",
                                               "Total"}) in KeepCols,
    ...
    MBScaled = Table.AddColumn(
        MBJoin, "Value", each let isAccrual = [BaseMetric] <> null, ftable = [F],
    ...
    /* =========================
       ...
       ========================= */
    PriorVisualLong = if PriorCont <> null and HasTimeCategory then
        let
            ...
            PriorLong = if HasPeriodLabel then
                ...
                let
                    ValCol = if List.Contains(Table.ColumnNames(PriorCsv), "Sum of Value") then "Sum of Value"
                             else if List.Contains(Table.ColumnNames(PriorCsv), "Value") then "Value" else null,
                    Filtered = if ValCol <> null then Table.SelectRows(PriorCsv, each [#"Time Category"] <> null and [PeriodLabel] <> null) else #table({}, {}),
                    Typed = if Table.RowCount(Filtered) > 0 then Table.TransformColumnTypes(Table.RenameColumns(Filtered, {{ValCol, "Value"}}), {{"Value", type number}}) else #table({}, {}),
                    ...
                    InWindow = Table.SelectRows(ExcludeCurrent, each List.Contains(PeriodLabelsMMYY, [PeriodLabel]))
                in
                    InWindow
            else
                #table({}, {})
        ...
        #table({"Time_Category", "PeriodLabel", "Value"}, {}),
               ...
               /* =========================
                  Combine long tables: Prior visual (historical) + Current ETL
                  (12-25 only)
                  ========================= */
               LegacyLong = Table.SelectColumns(LegacyWithLabelFiltered,
                                                {"PeriodLabel", "Time_Category",
                                                 "Value"}),
    ...
    HistoricalFromPrior = if Table.RowCount(PriorVisualLong) > 0 then PriorVisualLong else LegacyLongFiltered,

    // Current month from ETL (only 12-25)
    CurrentMonthETL = Table.SelectRows(Table.Combine({LegacyLongFiltered,
                                                      MBFinal}),
