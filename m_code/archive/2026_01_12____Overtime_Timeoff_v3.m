...
#table({"Metric", "Period", "Total"}, {}),
                ...
                let
                    ...
                    Filtered = if ValCol <> null
                               then Table.SelectRows(
                                   PriorCsv,
                                   each [#"Time Category"] <> null and [PeriodLabel] <> null)
                               else #table({}, {}),
    ...
    HistoricalFromPrior = if Table.RowCount(PriorVisualLong) >
                          0 then PriorVisualLong else LegacyLongFiltered,
    ...
    WithValue = Table.AddColumn(Joined, "Value",
                                each if [J] <> null and Table.RowCount([J]) > 0
                                     then [J]{0}[Value]
                                     else 0,
                                type number),
