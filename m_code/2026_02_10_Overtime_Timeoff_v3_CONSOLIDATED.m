...
// # Master_Automation/m_code/2026_02_10_Overtime_Timeoff_v3_CONSOLIDATED.m
// # Author: R. A. Carucci
// # Purpose: Single-query version: loads FIXED + monthly_breakdown + prior anchor in one let, outputs long table for Monthly Accrual and Usage Summary. Use this if staging queries cause "matches no exports."
// # Use in query: ___Overtime_Timeoff_v3. Output column for the visual is Time_Category (kept so existing visual binding works).
//
let
    // -------- 1. Load FIXED (latest CSV) --------
    OT_FIXED = let
    ...
    // -------- 3. Load prior anchor (for scaling) --------
    OT_Prior = let
        BasePath        = "C:\Users\carucci_r\OneDrive - City of Hackensack",
        AnalyticsFolder = BasePath & "\02_ETL_Scripts\Overtime_TimeOff\analytics_output",
        AFiles          = Folder.Files(AnalyticsFolder),
    ...
    // Keep column as Time_Category so existing visual binding works (no rename to "Time Category")
