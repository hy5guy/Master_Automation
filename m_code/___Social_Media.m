...
// # Author: R. A. Carucci
// # Purpose: Load rolling 13-month social media post counts by platform from
// STACP workbook for Power BI reporting.
...
#"Changed Type" = Table.TransformColumnTypes(_stacp_mom_sm_Table, TypeList),
    ...
    // Start of window = 12 months before EndMonth (inclusive, total = 13
    // months) Example: StartMonth = 2024-09-01  → months: Sep-2024 ... Sep-2025
    StartMonth = Date.StartOfMonth(Date.AddMonths(EndMonth, -12)),

    // Build ordered list of the 13 month keys in "MM-yy" format to match your
    // columns
    ...
    // Some workbooks may be missing a month or Total—keep only columns that
    // actually exist
    ExistingInOrder =
        List.Select(DesiredOrder, each List.Contains(ColumnNames, _)),
