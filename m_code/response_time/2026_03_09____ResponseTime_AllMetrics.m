let
    ...
    LoadCSV = (filePath as text) =>
        let
            raw = Csv.Document(
                File.Contents(filePath), [Delimiter = ",", Encoding = 65001, QuoteStyle = QuoteStyle.None]
            ),
    ...
    FilteredMetric = Table.SelectRows(
        AllData,
        each
            List.Contains(
    ...
    WithYearMonth = Table.AddColumn(
        ...
        each
            let
                parts = Text.Split(Text.Trim([#"MM-YY"]), "-"), mm = parts{0}, yy = "20" & parts{1}
            in
                yy & "-" & Text.PadStart(mm, 2, "0"),
        type text
    ),
    WithDateKey = Table.AddColumn(
        WithYearMonth, "Date_Sort_Key", each try Date.FromText([YearMonth] & "-01") otherwise null, type date
    ),
    WithMetricLabel = Table.AddColumn(
        WithDateKey,
        "Metric_Label",
        each
            if [Metric_Type] = "Time Out - Time of Call" then
                "Total Response"
            else if [Metric_Type] = "Time Out - Time Dispatched" then
                "Travel Time"
            else if [Metric_Type] = "Time Dispatched - Time of Call" then
                "Dispatch Queue"
            else
                [Metric_Type],
        type text
    ...
    WithMetricSort = Table.AddColumn(
        ...
        "Metric_Sort",
        each
            if [Metric_Type] = "Time Dispatched - Time of Call" then
                1
            else if [Metric_Type] = "Time Out - Time Dispatched" then
                2
            else
    ...
    WithMonthName = Table.AddColumn(
        ...
        each
            let
                ...
                name = if monthNum >= 1 and monthNum <= 12 then monthNames{monthNum - 1} & " " & parts{0} else [
                    YearMonth
                ]
            in
                name,
    ...
    WithRTSort = Table.AddColumn(
        ...
        "Response_Type_Sort",
        each
            if [Response_Type] = "Emergency" then
                1
            else if [Response_Type] = "Urgent" then
                2
            else if [Response_Type] = "Routine" then
                3
            else
                99,
        Int64.Type
