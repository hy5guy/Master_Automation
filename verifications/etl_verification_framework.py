...
Generic base class for verifying monthly ETL scripts.
Extracts common patterns from successful Overtime & Time Off verification.
...
class ETLVerifier:
    ...
    def parse_long_format(self, filepath: str) -> pd.DataFrame:
        """Parse long-format export (e.g., Overtime style)."""
        try:
    ...
    def parse_wide_format(self, filepath: str) -> pd.DataFrame:
        """
        Parse wide-format export and convert to long format.
        Handles quoted CSV lines with comma-separated values.
        """
        try:
            rows = []
    ...
    def load_current_export(self, filename: str) -> pd.DataFrame:
        """Load current month export from outputs directory."""
    ...
    def load_backfill_export(self, filename: str) -> pd.DataFrame:
        """Load previous month backfill for comparison."""
        # Try with backfill month prefix
    ...
    def compare_month_overlap(
        ...
        """
        Compare overlapping months between current and backfill exports.
    ...
    def check_data_integrity(self, df: pd.DataFrame, filename: str) -> List[Dict]:
        """
        Run common data integrity checks.
    ...
    def validate_processing_rules(self) -> List[Dict]:
        """
        Override in subclass with script-specific processing rules.
    ...
    def generate_report(self, comparison_results: Dict) -> str:
        """
        Generate standardized markdown verification report.
