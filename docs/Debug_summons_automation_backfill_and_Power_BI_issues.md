...
| Core ETL | `scripts/summons_etl_normalize.py` |
| Backfill merge | `scripts/summons_backfill_merge.py` |
| Power BI source | `03_Staging/Summons/summons_slim_for_powerbi.csv` |
| M code queries | `m_code/summons/` (summons_13month_trend, summons_all_bureaus, summons_top5_moving, summons_top5_parking) |

### Resolved (v1.18.0 / v1.18.1)

| Plan Item | Resolution |
|-----------|------------|
| A1: Power BI COUNT vs SUM(TICKET_COUNT) | `List.Sum([TICKET_COUNT])` replaces `Table.RowCount` in all_bureaus, top5_moving, top5_parking |
| A2: FilteredPreferBackfill drops 01-25 and 02-25 | Logic replaced with `IS_AGGREGATE = true`; `BackfillMonths = {}` |
| A1 (Part 2): YearMonthKey missing from ETL | `YearMonthKey` computed and included in slim output |
| WG2 filter dropping dept-wide trend | Filter removed |
| Table.RowCount vs TICKET_COUNT | Fixed in bureau and top5 queries |
| Two runner scripts | `run_summons_pipeline.py` deleted |
| Diagnostic scripts | 15 scripts archived to `02_ETL_Scripts/Summons/archive/` |
| summons_etl_enhanced.py reference | Docs corrected to reference actual scripts |
| Gap months | Narrowed to 07-25 only; backfill prefer months cleared |
| TYPE classification | Raw Case Type Code (M/P/C) from export; statute-based lookup removed |
| Source file | Switched from `summons_powerbi_latest.xlsx` to `summons_slim_for_powerbi.csv` |
...
| Item | Notes |
|------|-------|
| March 2025 (03-25) | Permanent gap — no e-ticket export exists; no backfill CSV supplied |
| M=406 vs M=462 (Jan 2026) | Methodology difference (statute-based vs Case Type Code); no manual audit to confirm |
| E-ticket delimiter (2025 vs 2026) | 2025 files semicolon-delimited; 2026 comma. ETL handles both via FIXED fallback; verify if all 2025 months load correctly |
| Part 1 issues (A3–A5) | Semicolon delimiter, Unicode logging, coalesce bug — may be addressed in current ETL; not re-verified |
